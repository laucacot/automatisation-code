...
const value_type e0=1.6022e-19; //Coulomb valeur de la charge elementaire
const value_type eps0=8.8542e-12; //s2.C2/kg/m3 permittivite du vide
const value_type cVfl=e0/(4.*pi*eps0); //champ electrique ??
const value_type V=100.; //V voltage applique

const value_type rNP=1.e-9; //m rayon des NP
const value_type qNP=0.; //C charge initiale des NP
const value_type nNP=2.e15; //m-3 densite initiale des NP

const value_type vSi=(4./3.)*pi*pow(1.36e-10,3); //m3 volume d'un atome de Si
const value_type tauxinj_ev=C/n_SiH4_ini; //taux d'injection et d'evacuation
...
value_type p1,p2,g1,g2,g3,g4,Tp,Tx,Tj,Fe,facte,factp,factn;
...
if (a!=4 or a!=10 or a<20){dndt[a]=dndt[a]-tauxinj_ev*n[a];}//pompe sur les especes non +
...
dndt[7]=dndt[7]+(ffn2+ffp4+ffn15+ffn16+ffn17)*nNP
		...
		+0.3*DA[4]*n[4];//SiH3+ + e -> Si + H + H2 sur paroi
dndt[8]=dndt[8]-ccol*coll[8]*n[8]
		+DA[4]*n[4];//SiH3+ + e -> SiH2 + H sur paroi
dndt[9]=dndt[9]+DA[10]*n[10]// H2+ + e -> H2 sur paroi 
		...
		+ccol*(2.*coll[5]*n[5]+coll[6]*n[6]+coll[9]*n[9]+2.*coll[11]*n[11]
			+coll[12]*n[12]+3.*coll[15]*n[15]);
...
// Densite des NP:
dndt[23]=dndt[23] + dndt[13] +dndt[17];
...
{ value_type DP=0.5*DL[0]*pow((V/L),2.);  //eV.eV/s puissance par unite de volume/ne*Te
  value_type Vfl=cVfl*n[22]/n[21];//V potentiel flottant e*qNP / 4 pi eps0 rNP
  value_type sNP=pi*pow(n[21],2); //surface de la NP
  if (Vfl<=0.){ Fe=sNP*vth[0]*exp(Vfl/Te);}
 ...
 return 
	...
	+k(10,Te)*n[6]*1.94 + k(11,Te)*n[6]*1.30 + k(12,Te)*n[2]*1.16
    	...
    	+k(44,Te)*n[17]*1.25
	+DA[0]*1.5*Te /*perte sur les parois*/
	+Fe*nNP*1.5*Te /*pertes sur les NP*/
	+tauxinj_ev*1.5*Te /*perte taux injection evacuation*/;
  }
